...
class TestCrashReconcile(unittest.TestCase):
    ...
    def test_reconcile_unconsumed_dev_manifest_after_crash(self):
        ...
        self.store.create_task("task-crash-1", "Crash Test", "feat", "main")
        self.store.update_task_state("task-crash-1", AgentState.CODING)

        # Physical disk has valid .dev.yml
        mock_exec = MockAgentAdapter("cc-ds4", "claude-code", role="executor")
        mock_exec.simulate_produce_dev_manifest(self.tmpdir, commit_sha="commit-recovered-01", review_round=1)

        reconciler = StateReconciler(self.store, project_root=self.tmpdir)
    ...
    def test_reconcile_vote_result_after_crash(self):
        ...
        self.store.create_task("task-crash-2", "Crash Test 2", "feat", "main")
        self.store.update_task_state("task-crash-2", AgentState.CONSENSUS_CHECK, checkpoint_ref="c-rev-02", review_round=1)

        # Physical disk has vote_result.json with APPROVED (2 Reviewer approvals)
        agg = VoteAggregator(project_root=self.tmpdir)
        ...
        mock_rev1.simulate_produce_review_manifest(
            self.tmpdir, checkpoint_ref="c-rev-02", review_round=1,
        ...
        agg.generate_vote_result("c-rev-02", "cc-ds4", 1, 2, collected, write_to_disk=True)
