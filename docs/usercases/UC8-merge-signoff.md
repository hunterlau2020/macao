...
- **关联**：PRD v2.5 §3.3（E4/E4a/E4b）、§14.5（Merge Policy）、§15.1；`execute_merge`（orchestrator.py:700）、Merge Controller 六道关卡；FAQ Q12/Q16。
...
### 关卡 6：源码推送与 Post-merge 归档（E4a）

push 前**最终硬校验**：待推对象 == `vote_result.checkpoint_ref`（字节级）；push 成功 → E4a → `DONE`：发 `MERGE_COMPLETED`（含 merge_commit）；执行 Post-merge 证据封存（PRD §14.5 第 5 步）并归档至 `.macao/archive/<ref>/r<round>/`，不污染 source 分支 HEAD；agmsg 通告全员（结果 + 归档路径）。
...
| E4a_pre | Pre-merge Evidence 校验未通过 | **不合并**；拦截至 E4b，提示推送 evidence ref 后重试 |
...
1. 六关卡顺序 + 单关卡注入失败（fixture：Pre-merge 未推送、CI 非零退出、签字拒绝、冲突仓库、ref 漂移）各自 → E4b 且 target HEAD 不变
2. Pre-merge Evidence 校验：`refs/macao/evidence/<task_id>/r<round>` 未推送或 `ls-remote` 失败时，严格禁止进入源码合并步骤（fail-closed 拦截）
3. ref 漂移检测：E4 后注入新 commit → push 前硬校验拦截（E2）
4. 签字：默认无 `merge approve` 不 push；`--auto-signoff` 审计 `signer=system-runner`（诚实性断言）
5. 归档完整性：E4a 后归档目录文件清单与 sha256 对账零差集
6. `MERGING` 与 `DONE` 区分：push 完成才 DONE；中断停在 `MERGING` 可重入（幂等续跑）
