...
| **UC-2** | 任务受理 | 管理员或执行者 | `IDLE` → `CODING`（E1）；`macao task create`；下发 Type B 信封 | [UC2-task-create.md](UC2-task-create.md) | 设计稿 (v2.5) |
...
| **UC-4** | 评审派发与审查 | 编排器（邮差）+ 各评审专家 | `READY_FOR_REVIEW` → `WAITING_REVIEW`（E2）；下发 Type C；专家写全文 + `.review.yml` | [UC4-review-dispatch.md](UC4-review-dispatch.md) | 设计稿 (v2.5) |
...
| **UC-6** | 意见处置与返工 | 执行者 | `CONSENSUS_CHECK` / `REWORK`；写独立 `executor.disposition.yml` 意见处置；按 `requires_new_checkpoint` 分流 E4 / E5a | [UC6-issue-triage-rework.md](UC6-issue-triage-rework.md) | 设计稿 (v2.5) |
...
| **UC-8** | 合并与签字 | 编排器 + 管理员 | `MERGING` → `DONE`（E4a）；五道关卡；`macao merge approve`；Git 引用与归档 | [UC8-merge-signoff.md](UC8-merge-signoff.md) | 设计稿 (v2.5) |
...
### UC-2 任务受理
编排器不规划、不拆 WBS。人（或执行者 CLI）提交标题、可测验收判据、分支；缺字段原子拒绝。编排器建 `tasks` 行并投递 `DEVELOPMENT_STARTED`（Type B）信封。
...
### UC-4 评审派发与审查
编排器是邮差：E2 把执行者已有 manifest 原样放入 `REVIEW_REQUEST`（Type C，零 base64，10 个必需与语义块）并 agmsg ping。专家在独立 worktree（`.macao/worktrees/<agent_id>/<task_id>/r<round>`）取 diff，全文进 `docs/reviews/*-review-result-<mid>-<reviewer>.md`，`.review.yml` 含总票 + 问题索引。
...
### UC-6 意见处置与返工
执行者读全文与 `issues_index`，输出独立 `executor.disposition.yml`（三态：`DRAFT` / `PENDING_ADMIN` / `FINAL`），逐项声明处置类型与必填布尔 `requires_new_checkpoint`。全 false 且 APPROVED 触发 E4 进 `MERGING`；任一 true 触发 E5a 进 `REWORK`；机器否决触发 E5 进 `REWORK`。

### UC-7 人工接管
适用：计票 Deadlock、init 无法唯一识别、覆盖上限仍返工、处置超时。选项闭合：`APPROVED` / `REWORK` / `RETRY_REVIEW` / `CANCEL`（支持 `--exempt-issue-ids`）。**DEADLOCK 时已即时落盘不可变 `vote_result.json`，管理员裁定写入独立 `admin_override.json`，严禁二次回写 `vote_result.json`**。

### UC-8 合并与签字
`MERGING` 合的是 git（检出 / ff_only / CI gate / 签字 / push），不是「合并评审意见」。评审对象 = 合并对象（checkpoint 硬绑定）。默认 `require_human_signoff`。合并成功后全部产物提升至 `refs/macao/evidence/<task_id>/r<round>` 并在 `.macao/archive/` 归档。
...
| `.macao/.reviews/<reviewer_id>.review.yml` | 各评审专家 (Reviewer) | 专家票面与问题索引信封（三值 `vote`、`opinion`、`issues[]`） |
| `.macao/vote_result.json` | 编排器 (Orchestrator) | 不可变机器计票与问题目录（三值 `decision`、`policy_snapshot`、`issues_index`、`requires_disposition`） |
| `.macao/executor.disposition.yml` | 执行者 (Executor) | 独立意见处置信封（`disposition_status: FINAL`、`requires_new_checkpoint: boolean`） |
