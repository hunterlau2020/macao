...
- **日期**：2026-08-28
- **验证版本**：Phase 0 / Phase 1 核心框架实现、Phase 1 PTY 实机联调与 Phase 2 端到端微任务协同闭环
- **测试结果**：**34 / 34 测试用例全部 PASS (100%)**
- **验证范围**：物理契约产物规范、共识多数仲裁算法、单进程事件循环与 Worktree 隔离机制、配置装配与消息广播独立投递、4 款真实 AI CLI（Claude Code / Codex / OpenCode / AGY）PTY 启停与进程树安全回收、端到端微任务全生命周期协同闭环。
...
| **Phase 1 实机** | **真实 AI CLI 进程生命周期与 PTY 交互、ANSI 码流捕获与进程树强杀安全回收** | `test_integ_harness.py`、`macao test-clis` | **✅ VERIFIED**<br>在 Linux 环境下对宿主真实安装的 Claude Code (`claude`)、Codex (`codex`)、OpenCode (`opencode`)、Google Antigravity (`agy`) 逐一验证 PTY 进程拉起、日志清洗与会话强杀，4/4 真实 CLI 均在 <1s 内完成启动并干净退出，实现 0 孤儿/0 僵尸进程残留。 |
| **Phase 2 协同** | **端到端微型任务协同流转（Task Start -> Coding Checkpoint -> 3-Reviewer Worktrees -> 2/3 仲裁 -> Fast-forward Merge -> DONE）** | `test_e2e_phase2.py`、`macao e2e-run` | **✅ VERIFIED**<br>全自动完成微型算术模块代码开发提交、3 方专属 Worktree 审查分发、2/3 多数多数票裁决、快进合并与目标分支 SHA 硬匹配校验（100% Match），产物非覆盖追加归档至 `.macao/archive/`。 |
