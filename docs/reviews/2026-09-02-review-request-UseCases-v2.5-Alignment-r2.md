# MACAO 全量用例体系（UseCases）PRD v2.5 规格与 D-1～D-9 架构裁定对齐复审申请（Round 2）
...
针对专家委员会对前序提交 `caf3473` 与 `5583bdd` 提出的全部阻断项（包括处置路径收敛 B-1、AEP Type 字母对齐 B-2、`items[]` 字段名 B-3、UC-8 Pre-merge Evidence 关卡 B-4、用例 YAML 契约合规 B-5、E7 豁免流唯一出口边 P1-1、D-1～D-9 权威编号逐字对齐 P1-2 等），本申请涵盖的全量 **13 份用例文档**已完成全面物理闭环，现提请专家委员会进行 Round 2 终局定级评审。
...
|---|---|---|---|
| 1 | [`docs/usercases/README.md`](../usercases/README.md) | 用例目录与主旅程总览 | 1. 确立主旅程流向：`UC-5 计票` $\rightarrow$ `UC-6 处置 (E5/E5a)` $\rightarrow$ `UC-8 合并 (E4/E4a)`；<br/>2. 建立 5 大核心产物单一垄断写者规范表（统一路径 `.macao/.dispositions/r<round>/executor.disposition.yml` 与 `items[]` 契约字段，消除重复行）。 |
...
| 3 | [`docs/usercases/UC1-init-glm.md`](../usercases/UC1-init-glm.md)<br/>[`UC1-init-gemini.md`](../usercases/UC1-init-gemini.md) | `macao init` / `adopt` | 1. 静态 `macao.yaml`（v2.5 全量加权与预算配置）与动态 `agent_registry` 分离；<br/>2. 依据工作区特征识别 10 态，推不出唯一态则问管理员；<br/>3. agmsg 仅作通知 short ping，正文零模型语义依赖；<br/>4. `h1`/`h2` 调度与投影表显式增补 DEADLOCK override 后等待 FINAL disposition 的 `SHOULD_DISPOSE` 投影；<br/>5. 统一 gitignore 9 规则与五重门禁纯整数公式。 |
...
| 9 | [`docs/usercases/UC7-human-override.md`](../usercases/UC7-human-override.md) | 管理员人工接管 (`override resolve`) | 1. **严格落地 D-1**：DEADLOCK 时 `vote_result.json` 不可变，管理员裁定写入独立 `admin_override.json`；<br/>2. 闭合 5 大选项（`APPROVED`, `REWORK`, `RETRY_REVIEW`, `CANCEL`, `EXTEND`）并支持 `--exempt-issue-ids` 局部豁免；<br/>3. **转移列与语义列严格同步**：`APPROVED` 落盘 override 解除 HOLD 并投影 `SHOULD_DISPOSE` $\rightarrow$ 经执行者提交 FINAL disposition 校验后触发 E4 $\rightarrow$ `MERGING`（严禁无 FINAL 直跳，严禁管理员代写）。 |
| 10| [`docs/usercases/UC8-merge-signoff.md`](../usercases/UC8-merge-signoff.md) | `MERGING` $\rightarrow$ `DONE` (E4a) | 1. 评审对象 = 合并对象（`checkpoint_ref` 硬校验）；<br/>2. **六道关卡顺序执行**：关卡 1 Pre-merge Evidence Push 校验（`ls-remote` 校验 `refs/macao/evidence/<task_id>/r<round>` 已推送） $\rightarrow$ 关卡 2 检出 $\rightarrow$ 关卡 3 技术合并 $\rightarrow$ 关卡 4 CI gate $\rightarrow$ 关卡 5 人工签字 $\rightarrow$ 关卡 6 源码推送与 Post-merge 证据封存归档；<br/>3. 杜绝“源码已合并而证据未落地”的审计断裂。 |
...
## 3. PRD v2.5 权威架构裁定（D-1 ～ D-9）逐条对齐表
...
综上所述，MACAO 全量用例体系已与 PRD v2.5 规格及 D-1～D-9 权威架构裁定达成高度一致，全部前序阻断项（包括 E7 豁免流唯一边、D-1～D-9 对照表对齐）均已实质闭环，具备严密的逻辑自洽性与机器可验证性。
