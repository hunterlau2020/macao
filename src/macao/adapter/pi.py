"""Pi Coding Agent CLI Adapter Implementation (PRD §12.3)."""
...
class PiAdapter(AgentAdapter):
    """Adapter for Pi Coding Agent CLI (Reviewer / Executor)."""

    def __init__(self, agent_id: str = "pi", config: Optional[Dict[str, Any]] = None):
        super().__init__(agent_id, "pi", config)
    ...
    def preflight(self) -> PreflightCheckResult:
        ...
        try:
            ...
            return PreflightCheckResult(
                ...
                remediation="Ensure pi is executable."
            )

    def start(self) -> bool:
        exe = self._resolve_binary() or "pi"
        cmd = [exe]

        # In review mode, default to read-only tool restrictions
